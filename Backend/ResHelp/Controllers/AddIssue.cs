    ...
    try
    {
        ...
        foreach (var doc in issuesSnapshot.Documents)
        {
            ...
            issuesList.Add(new RequestDto
            {
                ...
                Category = data.GetValueOrDefault("Category")?.ToString() ?? string.Empty,
                Priority = data.GetValueOrDefault("Priority")?.ToString() ?? string.Empty,
                Location = data.GetValueOrDefault("Location")?.ToString() ?? string.Empty,
                IsUrgent = data.GetValueOrDefault("IsUrgent") != null && (bool)data["IsUrgent"],
                Status = data.GetValueOrDefault("Status")?.ToString() ?? "Pending",
                CreatedAt = data.GetValueOrDefault("ReportedAt") switch
                {
                    Google.Cloud.Firestore.Timestamp ts => ts.ToDateTime().ToString("o"),
                    string s => s,
                    _ => DateTime.UtcNow.ToString("o")
                },
                ReporterEmail = data.GetValueOrDefault("ReporterEmail")?.ToString() ?? string.Empty,
                ReporterName = data.GetValueOrDefault("ReporterName")?.ToString() ?? string.Empty,
            });
